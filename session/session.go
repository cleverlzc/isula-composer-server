...
// InitSession sets the configs, for example the cacheDirectory
func InitSession(sc config.SessionConfig) error {
	if sc == nil {
		return errors.New("session config should not be empty")
	}

	v, ok := sc["filesystem"]
	if !ok {
		return errors.New("only filesystem backend supported in `session`")
	}

	cacheDir, ok := v["cacheDirectory"]
	if !ok {
		return errors.New("`cacheDirectory' must be set")
	}

	rootCacheDir = cacheDir.(string)
	return nil
}
